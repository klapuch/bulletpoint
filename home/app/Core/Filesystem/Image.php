...
final class Image {
	...
	public function resize(Size $newSize): self {
		...
		imagecopyresampled($image, $source, 0, 0, 0, 0, $newSize->width(), $newSize->height(), $width, $height);
	...
	private function toReadableExtension(array $extensions): string {
		return rtrim((string)array_reduce(
			$extensions, function($previous, $extension) {
