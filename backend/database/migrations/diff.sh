...
git --no-pager diff --diff-filter=A --name-status master.. -- $BASE_DIR/backend/database/migrations/**/*.sql | sed -e "s~^A[[:space:]]~$BASE_DIR/~g" | xargs --no-run-if-empty --max-lines=1 sh ${0%/*}/run.sh
