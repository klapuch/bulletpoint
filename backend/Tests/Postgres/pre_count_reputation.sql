...
BEGIN
	...
	PERFORM assert.same(1, (SELECT reputation FROM bulletpoint_reputations));
...
BEGIN
	...
	PERFORM assert.same(2, (SELECT reputation FROM bulletpoint_reputations));
END $BODY$ LANGUAGE plpgsql VOLATILE;
...
BEGIN
	...
	PERFORM assert.same(4, (SELECT reputation FROM bulletpoint_reputations LIMIT 1 OFFSET 0));
	PERFORM assert.same(4, (SELECT reputation FROM bulletpoint_reputations LIMIT 1 OFFSET 1));
END $BODY$ LANGUAGE plpgsql VOLATILE;
...
BEGIN
	...
	PERFORM assert.null((SELECT reputation FROM user_tag_reputations WHERE tag_id = v_tag_id1));
	PERFORM assert.same(2, (SELECT reputation FROM user_tag_reputations WHERE tag_id = v_tag_id2));
	PERFORM assert.same(2, (SELECT reputation FROM bulletpoint_reputations LIMIT 1 OFFSET 0));
	PERFORM assert.same(2, (SELECT reputation FROM bulletpoint_reputations LIMIT 1 OFFSET 1));
END $BODY$ LANGUAGE plpgsql VOLATILE;
