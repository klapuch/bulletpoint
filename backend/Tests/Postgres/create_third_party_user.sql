...
BEGIN
	PERFORM assert.same(ARRAY['1', 'foo@facebook.com'], (SELECT ARRAY[facebook_id, email] FROM create_third_party_user('facebook', '1', 'foo@facebook.com')));
...
BEGIN
	...
	PERFORM assert.same(ARRAY['1', 'bar@facebook.com'], (SELECT ARRAY[facebook_id, email] FROM create_third_party_user('facebook', '1', 'bar@facebook.com')));
