...
final class PlPgSqlCheck implements Scheduling\Job {
	...
	public function fulfill(): void {
		...
		echo sprintf('<error>Number of errors: %d</error>', $numberOfErrors) . PHP_EOL;
		exit(1);
	}
	...
	private static function writeErrors(array $errors): int {
		foreach ($errors as $error) {
			...
			echo sprintf('<info>Function: %s</info>', $function) . PHP_EOL;
	...
	private function parsedFunctionErrors(array $errors): array {
		$parsed = array_map(function (array $error): array {
			...
			foreach ($xml->xpath('Issue') as $issue) {
				...
				$descriptions[] = array_map('strval', [
					...
					'line' => current($issue->xpath('Stmt')) === FALSE ? '' : current($issue->xpath('Stmt'))->attributes()['lineno'] ?? '',
				]);
			}
